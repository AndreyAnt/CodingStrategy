...
//  
//
//  Created by andrey.antropov on 02.02.2021.
//
...
public struct NestedKey<T: Decodable>: Decodable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        let lastLeaf = try nextKeys.indices.dropLast().reduce(container) { (nestedContainer, keyIdx) in
            do {
                ...
                try NestedKey.keyNotFound(key: key, context: context, container: container, nextKeys: nextKeys[..<keyIdx])
            }
        }
        // key leaf
        do {
            ...
            try NestedKey.keyNotFound(key: key, context: context, container: container, nextKeys: nextKeys.dropLast())
        }
    }
    ...
    private static func keyNotFound<C: Collection>(
        ...
        nextKeys: C
    ) throws -> Never where C.Element == String {
